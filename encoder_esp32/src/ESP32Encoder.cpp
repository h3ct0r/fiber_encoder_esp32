...
ESP32Encoder::ESP32Encoder(int16_t CPR, int32_t positionComputeInterval, bool always_interrupt_, enc_isr_cb_t enc_isr_cb, void *enc_isr_cb_data) : CPR{CPR}, positionComputeInterval{positionComputeInterval}, always_interrupt{true}, aPinNumber{(gpio_num_t)0}, bPinNumber{(gpio_num_t)0}, unit{(pcnt_unit_t)-1}, countsMode{2}, count{0}, r_enc_config{}, _enc_isr_cb(enc_isr_cb), _enc_isr_cb_data(enc_isr_cb_data), attached{false}, direction{false}, working{false}, wheelDiameter{0.385}, lastCountTime{0.0} {
...
void ESP32Encoder::setCPR(int64_t value) {
    if (value <= 0) return;
